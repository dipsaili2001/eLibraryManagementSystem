...
namespace eLibraryManagementSystem
{
    public partial class adminbookissuing : System.Web.UI.Page
    {
        ...
        void updateFine(double NrOfDays)
        {
            SqlConnection con = new SqlConnection(strcon);
            if (con.State == ConnectionState.Closed)
            {
            ...
            SqlCommand cmd = new SqlCommand("UPDATE book_issue_tbl set fine = '" + NrOfDays.ToString() + "'", con);
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (NrOfDays > 0) // To add fine in database
                    {
                        //updateFine(NrOfDays);
                        //GridView1.DataBind();
                        //Response.Write("<script>alert('" + NrOfDays + "');</script>");
                    }
                    
                    if (today > dt)
                    {
                        double fineCal = NrOfDays * 10.0;
                        e.Row.Cells[6].Text = "Rs " + fineCal.ToString() + "/-";
